...
@NamedQueries({
		...
		@NamedQuery(name = "empDepAlias", query = "select e.officeCode , e.name, CASE When (e.officeCode = 1) THEN 'IT'"
				+ "WHEN (e.officeCode = 6) THEN 'Admin'" + "WHEN (e.officeCode = 5) THEN 'HR'"
				+ "WHEN (e.officeCode = 4) THEN 'Developers'" + "WHEN(e.officeCode = 3) THEN 'Accounts'"
				+ "WHEN (e.officeCode = 2) THEN 'Finanace'" + "ELSE 'General' END FROM Employee e") })
